...
#load image
carimage = pygame.image.load('car3.png')
...
yellow_strip = pygame.image.load('white_strip.png')
...
def game_loop():
    ...
    while not bumped:
        ...
        if y < obs_y + enemy_height:
            if x > obs_x and x < obs_x + car_width or x + car_width > obs_x and x + car_width < obs_x + enemy_width:
                screen.blit(render_text,(220,240))
                pygame.display.update()
                time.sleep(5)
                game_loop()
